...
public class XenditPaymentService implements PaymentService {
    ...
    public void handleNotification(PaymentNotification paymentNotification) {
        ...
        Order order = orderRepository.findByXenditInvoiceId(invoiceId)
                .orElseThrow(() -> new ResourceNotFoundException("Order not found for xendit with invoice ID: " + invoiceId));

        order.setXenditPaymentStatus(status);
        switch (status) {
            case "PAID":
                order.setStatus(OrderStatus.PAID);
                emailService.notifySuccessfulPayment(order); // Dibuat async agar webhook handleNotification tidak menunggu ketika retry
                trackPurchasedOrder(order); // Track user activity product purchase
                break;
            case "EXPIRED":
                order.setStatus(OrderStatus.CANCELLED);
                emailService.notifyUnsuccessfulPayment(order); // Dibuat async agar webhook handleNotification tidak menunggu ketika retry
                break;
            case "FAILED":
